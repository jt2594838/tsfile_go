...
func (d *IntRleDecoder) ReadInt() int {
	if !d.isReadingBegan {
...
func (d *IntRleDecoder) readPackage() {
	...
	case RLE:
		...
		d.currentValue = d.packageReader.ReadIntLittleEndianPaddedOnBitWidth(d.bitWidth)
...
func (d *IntRleDecoder) readBitPackingBuffer(bitPackedGroupCount int, lastBitPackedNum int, bitWidth int) {
	...
	d.decodedValues = make([]int, bitPackedGroupCount*conf.RLE_MIN_REPEATED_NUM)
	d.packer.UnpackAllValues(bytes, 0, bytesToRead, d.decodedValues)
}
